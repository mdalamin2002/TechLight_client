...
import { Hash, User, Calendar, Search, Settings } from "lucide-react";
...
const ReviewsTab = () => {
  ...
  // ===== Load data =====
  useEffect(() => {
    axios.get("/Moderator_Users_Reviewtab.json")
  ...
  const handleApprove = (id) => {
    setData(prev => prev.map(item => item.reviewId === id ? { ...item, status: 'Approved' } : item));
  };

  const handleRemove = (id) => {
    setData(prev => prev.map(item => item.reviewId === id ? { ...item, status: 'Removed' } : item));
  };
  ...
  const columns = [
    ...
    columnHelper.accessor("rating", {
      cell: (info) => <span className='text-yellow-500 font-semibold'>{info.getValue()} ⭐</span>,
      header: "Rating",
    }),
    ...
    columnHelper.accessor("status", {
      cell: (info) => {
        ...
        const color = status === 'Pending' ? 'bg-yellow-400' : status === 'Approved' ? 'bg-green-500' : 'bg-red-500';
        return (
            ...
            {table.getRowModel().rows.map((row, i) => (
              <tr key={row.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-indigo-50/40'} hover:bg-indigo-100/70 transition-colors`}>
                ...
                <td className="py-3 px-4 flex gap-2">
                  <button
                    onClick={() => handleApprove(row.original.reviewId)}
                    className="px-3 py-1.5 bg-green-500 text-white rounded-lg text-sm hover:bg-green-600"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => handleRemove(row.original.reviewId)}
